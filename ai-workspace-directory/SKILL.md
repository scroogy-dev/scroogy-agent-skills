...
ACME 결제 서비스를 구성하는 멀티 repo 워크스페이스.
결제 API, 모바일 클라이언트, 레거시 웹 결제 페이지를 한 곳에서 본다.
이 파일은 building 안내판이며, 도메인 본문은 각 floor의 `.ai/AI-CONTEXT.md`로 라우팅한다.
...
- 커밋 메시지: 각 floor의 `.ai/10_rules/` 또는 루트 `/git-commit` 스킬 참조
- PR 정책: 각 floor의 `.ai/10_rules/` 참조
...
- 로비 정보와 floor 정보 충돌 시 floor 우선.
- floor 정보와 소스 코드 충돌 시 소스 코드 우선.
- `placeholder` floor를 만나면 `ai-workspace` 스킬로 안내도 생성을 안내한다.
- `archived` floor에 대한 요청은 사용자 확인 후에만 진행한다.
````
...
| mobile-app | 존재 | **미등록** | drift: 로비 Floors에 추가 필요 |
...
- [위배] Floors 테이블에 `owner` 열 추가 → 4열로 정규화 예정
...
- 라이선스: 워크스페이스 루트 `LICENSE`
- 커밋·PR 정책: 각 floor의 `.ai/10_rules/` 참조
...
- 형식 위배 4건 (frontmatter, last updated 누락, Floors 5열, H1 헤더)
