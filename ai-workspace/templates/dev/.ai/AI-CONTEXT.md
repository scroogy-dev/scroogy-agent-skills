...
코드 작업(수정, 리뷰, 분석 등) 전에 `.ai/60_codebase/index.md`가 존재하면 참조하여 관련 기능의 엔트리포인트와 호출 흐름을 빠르게 파악합니다.
단, **SSoT(Single Source of Truth)는 소스코드**입니다. 색인은 오래되었을 수 있으므로 참고용으로만 활용하고, 반드시 실제 소스코드를 확인합니다.
