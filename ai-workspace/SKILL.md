...
| `## 프로젝트 규칙` 섹션 + 3열 표 | `## 프로젝트 규칙` 섹션과 3열 표(파일·설명·사용 시점)가 존재? | 상태별로 조치한다 — **섹션 부재**: `## 프로젝트 목적` 바로 다음에 프로파일별 템플릿 골격(인라인 규칙 안내 주석 + 선행 적용 문구 + 3열 표, 템플릿의 `## 프로젝트 규칙` 섹션과 동일)을 삽입한다. **섹션은 있고 표 부재**: 기존 인라인 본문을 그대로 둔 채 섹션 끝에 프로파일별 3열 표(템플릿의 표와 동일)를 삽입한다. **2열(파일·설명) 구버전 표**: 헤더·구분선에 `사용 시점` 열만 추가하고 기존 행의 해당 셀은 `<사용 시점>` placeholder로 두어 사용자가 채운다 — 이 검사는 **열 확장만** 수행하며 기존 표에 기본 행을 보충하지 않는다 (표준 2행은 아래 두 행 검사가, 구버전 기본 행 복원은 [구버전 구조 마이그레이션](#구버전-구조-마이그레이션)이 담당). 기존 인라인 규칙·사용자 작성 행은 보존한다. 아래 두 행 검사는 이 검사로 섹션·표 존재가 보장된 뒤에 수행한다. |
| `## 프로젝트 규칙` 표의 `context-loading.md` 행 | `## 프로젝트 규칙` 표에 `.ai/10_rules/context-loading.md` 행이 존재? | 표 끝에 표준 행을 삽입한다 — 파일 `.ai/10_rules/context-loading.md`, 설명 "작업 전 컨텍스트 확인 절차", 사용 시점은 프로파일별로 dev "코드·문서 작업 전" / doc "문서 작업 전" (AI-CONTEXT 템플릿의 해당 행과 동일). 기존 사용자 작성 행은 보존한다. 버전 고정 파일(update-1에서 무조건 덮어쓰기)은 이 행 검사로 안내도 참조까지 함께 보장한다. |
| `## 프로젝트 규칙` 표의 `writing-principles.md` 행 | `## 프로젝트 규칙` 표에 `.ai/10_rules/writing-principles.md` 행이 존재? | 표 끝에 표준 행을 삽입한다 — 파일 `.ai/10_rules/writing-principles.md`, 설명 "산출물 작성 원칙 (소스 코드 미적용)", 사용 시점 "산출 문서·PR·이슈·리뷰 코멘트 작성 시" (AI-CONTEXT 템플릿의 해당 행과 동일, dev·doc 동일 문구). 기존 사용자 작성 행은 보존한다. |
