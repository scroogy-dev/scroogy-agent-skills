...
**이동 제외 — 루트 상주 파일:** 템플릿 골격이 디렉토리 루트에 배포하는 파일(각 디렉토리의 `index.md`, `40_domain/glossary.md`)은 이동·`legacy/` 대상에서 제외하고 루트에 그대로 둡니다. 이 파일들은 항목이 아니라 디렉토리의 진입점이라, 아래 판단 기준을 그대로 적용하면 판정 앵커·키워드가 없다는 이유로 하위 디렉토리나 `legacy/`로 밀려 `context-loading.md`가 가리키는 공통 진입점이 깨집니다.
