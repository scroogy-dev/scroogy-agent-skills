...
요구사항·DoD 항목에 결속될 때만으로 한정한다. 스펙이 요구하지 않은 개선 제안은 1단계 판정을 낮추지 않고
2단계 낮음(LOW)·정보(INFO) 발견으로만 남긴다 — 종료 기준 없이 판정을 낮추면 감사가 끝나지 않는다.
...
- **발견 번호 계승** — 발견 번호(`F-n`)는 이슈 단위로 이어 붙인다. 재감사에서 `F-1`부터 다시 시작하지 않고,
  이전 리포트의 마지막 번호 다음부터 부여한다 — 번호를 매번 새로 매기면 잔여분이 새 문제처럼 보인다.
