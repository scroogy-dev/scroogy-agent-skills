...
description: PR 리뷰 코멘트를 수집·분류하고 항목별 의견(조치 필요/조치 불필요/수용(known issue)/코멘트로 충분/확인 필요)을 제시한 뒤, 사용자 선택에 따라 답글 게시 또는 코드 변경으로 대응합니다. 답글 게시·스레드 resolve·push는 외부 공개 행위이므로 승인 게이트를 생략하지 않습니다. PR 코멘트 처리, 리뷰 코멘트 대응·검토, 리뷰 반영, 피드백 반영·대응, 리뷰 답글 작성, pr feedback, review comments 처리 시 사용합니다.
---
...
- 보류·수용(known issue) 항목과 남은 리스크·후속 액션은 접지 않고 본문에 유지합니다 ("산출물 접기 기준" 참조) —
  수용 항목은 수용 근거(발생확률·영향도)를 함께 기록합니다.
