...
- **스킬 고유 추가 참조**:
  ...
  - `.ai/70_ledger/index.md` — 원장 등재를 선택한 항목이 있을 때만 읽습니다. 같은 지적이 이미 등재돼 있는지 대조하고 새 항목의 번호를 채번하는 데 씁니다 ("항목별 선택" 참조)
...
- 같은 지적이 원장 `active/`에 **`수용` 상태로** 있으면 새로 등재하지 않고 항목의 **재검토 조건 충족 여부**로 갈라 처리합니다 —
  ...
  - **조건 충족 + 종결**: 아래 종결 규칙(상태 변경 + `archive/` 이관)을 따릅니다.
...
- 기존 항목의 **종결**도 새 번호를 따지 않고 그 항목을 갱신합니다 — 상태를 바꾸고 `active/`에서 `archive/`로 옮긴 뒤 index 항목 목록을 갱신합니다.
  - 같은 지적을 이번 PR에서 **코드 수정**으로 처리해 원인이 사라졌으면 `해소(PR #<번호>)`.
  - 재확인 결과 지금 고쳐야 한다고 판단해 별도 이슈로 올렸으면 `승격(이슈 #<번호>)`.

  종결을 빠뜨리면 이미 고친 부채가 `active/`에 남아 이후 감사가 그것을 기등재 억제 기준으로 삼습니다.
- 원장 파일 생성 자체는 로컬 변경이라 아래 "승인 게이트 (생략 불가)"의 외부 공개 행위가 아닙니다.
  함께 선택한 답글 게시와, 등재 파일을 PR 브랜치에 담는 커밋·push는 그대로 승인 게이트를 거칩니다.
...
원장 등재는 로컬 파일 작업이라 위 매핑에 해당하지 않습니다 — 등재 절차는 "항목별 선택"의 등재 규칙을 따르고,
`gh`·MCP 가용 여부와 무관하게 수행합니다.
