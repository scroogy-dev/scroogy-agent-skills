...
- **동작**:
  ...
  2. **피드백만 제시**: 발견사항별로 동의 / 부분동의 / 반론, 위험도 재평가, 보정 시 **영향 파일 목록**을 제시한다. **이 단계에서는 파일을 수정하지 않는다.**
  3. **처리 방향 표**: 보정 대상과 처리 방향(반영 / 이관 / 보류)을 표로 정리해 보여준다.
  4. **항목별 승인 질의**: 사용자에게 **항목 단위로** 실행 여부를 확인받는다.
  5. **승인분만 보정**: 승인된 항목만 보정을 진행하고, 미승인 항목은 보류하거나 별도 이슈로 이관한다. 보정 결과는 `issue-<번호>-summary.md`에 반영한다 — 해당 Task 블록의 `audit 발견`에 그 Task 관련 발견 건수를, `보정 반영`에 **이 단계에서 승인을 통과해 실제로 보정한 건수**를 적는다. 미승인·보류·이관 항목은 `보정 반영`에 세지 않으므로, 두 값의 차이가 승인 게이트에서 걸러진 양이 된다.
