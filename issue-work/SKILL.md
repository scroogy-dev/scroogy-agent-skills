...
- **동작**:
  ...
  5. **승인분만 보정**: 승인된 항목만 보정을 진행하고, 미승인 항목은 보류하거나 별도 이슈로 이관한다. 보정 착수에도 같은 순서 게이트를 적용한다 — 1단계 승인분 보정을 마친 뒤에 2단계 승인분 보정에 들어간다. 보정 결과는 `issue-<번호>-summary.md`에 반영한다 — 해당 Task 블록의 `audit 발견`에 그 Task 관련 발견 건수를, `보정 반영`에 **이 단계에서 승인을 통과해 실제로 보정한 건수**를 적는다. 미승인·보류·이관 항목은 `보정 반영`에 세지 않으므로, 두 값의 차이가 승인 게이트에서 걸러진 양이 된다.
