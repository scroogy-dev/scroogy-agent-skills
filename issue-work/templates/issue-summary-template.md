...
형식: "벤더, 모델명". 계획·구현·audit 어느 칸이든 어느 벤더·모델이든 가능하다
...
모델 열의 "벤더, 모델명" 형식은 바꾸지 않고 effort는 이 열에만 둔다.
벤더가 다르면 같은 이름의 단계라도 같은 수준이라는 보장이 없으므로 effort는 벤더끼리 비교하지 않는다.
effort는 게이트 조건에 쓰지 않는다. 아래 벤더 교차 조건은 모델 열로만 확인하며, effort가 `-`여도 조건 판정과 Task N 완료에 영향이 없다.
...
| 구분 | 모델 | effort |
|------|------|--------|
| 계획 모델 | <!-- spec·plan을 쓴 모델. 형식: 벤더, 모델명 --> | <!-- 도구가 기록한 표기 그대로. 확인 불가면 - --> |
| 계획 audit 모델 | <!-- 계획 감사(issue-audit --plan)를 수행한 모델. 계획 모델과 다른 벤더. 형식: 벤더, 모델명. 계획 감사를 건너뛰면 `생략` --> | <!-- 계획 감사 리포트의 감사 effort 줄 값. 사용자가 기록. 확인 불가면 -, 계획 감사를 건너뛰면 `생략` --> |
| 구현 모델 | <!-- Task를 구현한 모델. 형식: 벤더, 모델명. 계획 모델과 같으면 같은 값 --> | <!-- 도구가 기록한 표기 그대로. 확인 불가면 - --> |
| 최종 audit 모델 | <!-- 구현 모델과 다른 벤더 모델. 형식: 벤더, 모델명. 마지막 교차모델 audit Task에서 사용자가 기록 --> | <!-- 최종 감사 리포트의 감사 effort 줄 값. 사용자가 기록. 확인 불가면 - --> |
