...
| 구분 | 모델 |
|------|------|
| 계획 모델 | <!-- spec·plan을 쓴 모델. 형식: 벤더, 모델명 --> |
| 계획 audit 모델 | <!-- 계획 감사(issue-audit --plan)를 수행한 모델. 계획 모델과 다른 벤더. 형식: 벤더, 모델명. 계획 감사를 건너뛰면 `생략` --> |
| 구현 모델 | <!-- Task를 구현한 모델. 형식: 벤더, 모델명. 계획 모델과 같으면 같은 값 --> |
...
아래 Task 블록의 지표 4종(`audit 발견`·`보정 반영`)과 별개다. 그 지표는 구현 Task 축이라 `summarize-metrics.sh` 집계 대상이고, 이 줄은 집계하지 않는다.
...
아래 스니펫이 동작하도록 `- **<필드>**: <숫자><단위>` 표기를 지킨다 (줄 시작, 굵게, 콜론 뒤 숫자).
  ...
  grep -E '^- \*\*(수행 모델|audit 발견|보정 반영|재시도)\*\*:' issue-<번호>-summary.md
-->
