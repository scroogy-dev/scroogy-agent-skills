...
> 다른 게이트의 선행 조건이 되는 항목은 그 의존 관계를 본문 문장에 한 구절로 남기고, 근거는 접기에 둡니다.
...
- **완료 기준**:
  ...
  - [D] <검사할 불변 조건 — 무엇이 보장되는지 한 문장>
    ...
    - 설계 주의: 임계값·기대 출력을 접기 제목이나 주석에 남깁니다 (예: 출력 0건, `grep -c 'X' file` ≥ 1, 테스트 0 실패).
    </details>
...
- **완료 기준**:
  - [D] <검사할 불변 조건 — 무엇이 보장되는지 한 문장>
    ...
    ```bash
    # 예시: 최근 커밋 제목이 Conventional Commits 형식인가
    git log -1 --pretty=%s \
      | grep -qE '^(feat|fix|docs|refactor|test|chore)(\(.+\))?!?: ' \
      || echo '위반: 커밋 제목 형식 불일치'
    ```
...
- **완료 기준**:
  ...
  - [D] summary의 Task 헤더 집합이 plan과 일치 — 블록 전체 누락·Task N 누락·빈 summary·두 경로 오기 차단. 아래 두 게이트의 선행 조건이다
    <details>
    <summary>검증 명령 — repo 루트에서 실행, 출력 0건이면 통과</summary>

    ```bash
    P=.ai/90_issues/active/issue-<번호>/issue-<번호>-plan.md
    S=.ai/90_issues/active/issue-<번호>/issue-<번호>-summary.md
    ...
    - 의존 근거: 아래 두 게이트는 summary에 실재하는 블록의 행만 검사하므로, 블록 자체가 없으면 검사 대상이 사라져 그대로 통과한다.
    </details>
  - [D] 이 검증 전에 Task 0 및 모든 일반 실행 Task의 summary `결과`가 완료·부분 완료·스킵 중 하나로 확정 — Task N 제외 블록마다 유효 `결과` 행 정확히 1개, 무효·중복 0건. 아래 `수행 모델` 게이트의 선행 조건이다
    <details>
    ...
    - 의존 근거: 비워 두면 "완료·부분 완료면 `수행 모델` 값 필수" 조건이 발동하지 않아 아래 강화 조건이 공집합이 된다.
    </details>
  - [D] 완료·부분 완료 Task의 `수행 모델`이 비어 있지 않고 `-`도 아닌 행 정확히 1개(`-`는 미착수·스킵 전용). 아래 교차 벤더 조건의 선행 조건이다
    <details>
    <summary>검증 명령 — repo 루트에서 실행, 출력 0이면 통과</summary>
    ...
    - 의존 근거: 전부 `-`로 남기면 아래 교차 벤더 조건이 공집합이 되어 우회가 된다.
    </details>
