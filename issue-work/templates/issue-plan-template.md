...
- **완료 기준**: `[D]` 검증 명령 전부 통과 + summary `모델 기록` 표의 `구현 모델`·`audit 모델` 두 행이 서로 다른 벤더로 채워진다("벤더, 모델명" 형식). 구현에 여러 벤더가 관여했으면 audit 모델의 벤더는 Task 0 및 일반 실행 Task의 비어 있지 않은 모든 `수행 모델` 벤더와도 달라야 한다 — 표의 대표값 비교만으로는 audit 벤더의 구현 참여를 놓친다.
