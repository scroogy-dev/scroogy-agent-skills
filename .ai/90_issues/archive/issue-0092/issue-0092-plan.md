...
- **작업 내용**:
  ...
  2. 판정은 `## 문항` 아래의 모든 `### ` 헤더를 문항 후보로 열어 블록을 나누고, 블록 단위로 R1~R6을, 파일 전체로 R7을 센다. 인식 범위 밖 헤더를 버리면 그 블록의 검사가 통째로 빠진다. `<details>` 안팎 판정은 태그 등장 순서로 접기 깊이를 세며, 한 행에 여러 태그가 있어도 순서대로 처리한다. 정답 블록은 존재·위치뿐 아니라 내용까지 본다. 블록 안 첫 내용 행의 형식(객관식 `(<문자>). <해설>`, 주관식 모범 답안), 정답 문자와 선택지 라벨의 결속, `근거: ` 행, 비즈니스 문항의 `정책 근거: ` 행을 판정한다. 첫 내용 행은 빈 행·태그 행과 한 행짜리 HTML 주석을 뺀 첫 행이며, 정답 문자 뒤에 공백만 있으면 해설 없음으로 본다. 렌더링되지 않는 자리표시자를 내용으로 세면 해설이 비어 있는 산출물이 게시로 넘어간다.
  ...
  4. `tests/fixtures/`: 정상 fixture 2개 이상 (대화형 산출 예시: 응답 기록 있음 / 댓글 본문 예시: permalink 있고 응답 기록 없음)과 R1~R7 반례 각 1개 이상 (예: R1 번호 건너뜀·관점 오기, R2 위치 행 없음·헤더 직후가 위치 행이 아님, R3 힌트 블록 2개·0개, R4 정답 블록이 힌트 앞, R5 객관식 선택지 1개·주관식에 선택지, R6 접기 밖 `정답` 행, R7 `## 문항` 없음·문항 0개·응답 기록이 문항 사이에 위치). 정답 블록 내용 반례도 함께 둔다 (빈 정답 블록, 객관식 첫 행 형식 이탈, 정답 문자 뒤 해설이 공백뿐, `근거: ` 행 누락, 주관식 모범 답안 누락, 모범 답안 자리가 HTML 주석뿐, 비즈니스 `정책 근거: ` 행 누락, 선택지에 없는 정답 문자). 러너는 템플릿 파일도 정상 fixture로 실행한다.
...
- **작업 내용**:
  ...
  6. `## 문항 생성 규칙`: 근거 규칙 (변경 위치 필수, 비즈니스 문항은 문서 근거 필수, 일반 지식 문항 금지, 정답·해설은 소스의 실제 동작과 대조하고 판단이 서지 않으면 최소 재현으로 확인, 소재는 기능 구현부·정책 우선), 테크 소재 축(git-review 7 카테고리), 힌트 규칙(정답 직접 언급 금지, 볼 위치·조건만), 정답 블록 형식(Task 1의 4번), 삭제 코드는 ` (base)` 표기.
...
- **작업 내용**:
  ...
  4. 시험에서 드러난 절차 결함(문항 소재 쏠림, 힌트가 정답 노출, 위치 줄 범위 오류, 정답·해설의 서술과 실제 동작 불일치)은 SKILL.md 규칙을 보정하고 summary `특이 사항`에 남긴다.
- **완료 기준**:
  - [D] 두 산출 파일이 형식 검사를 통과하고 문항 3개 이상·응답 기록 3행 이상이며, 댓글 본문의 **모든** 위치 행에 기준 커밋 SHA permalink가 있다  (spec DoD 5 명령과 동일)
    <details>
    <summary>검증 명령 — repo 루트에서 실행, 출력 0건이면 통과</summary>
  ...
  - [QD] 모든 문항이 변경 자체를 묻고 힌트가 정답을 직접 말하지 않으며, 정답 블록의 근거 위치가 diff와 일치하고 정답·해설의 서술이 소스의 실제 동작과 일치한다  (spec DoD 8과 같은 기준. 검증: 교차모델 audit이 산출물을 `gh pr diff 91`·실제 동작과 대조 채점)  ← 강등 사유: 의미 판단
...
- **완료 기준**:
  ...
  - [D] summary의 Task 헤더 집합이 plan과 일치한다. 블록 전체 누락·Task N 누락·빈 summary·두 경로 오기를 차단하며, 아래 두 게이트의 선행 조건이라 블록 자체가 없으면 그 두 게이트는 검사 대상이 사라져 그대로 통과한다
    ...
    ```bash
    P=.ai/90_issues/archive/issue-0092/issue-0092-plan.md
    S=.ai/90_issues/archive/issue-0092/issue-0092-summary.md
