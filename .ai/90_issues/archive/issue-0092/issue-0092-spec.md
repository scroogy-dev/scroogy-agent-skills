...
| 규칙 | 내용 |
|------|------|
| R1 | `## 문항` 아래의 모든 `### ` 헤더가 문항이며 `### Q<n> [<관점> · <형식>]` 한 줄이다. `<관점>`은 `비즈니스`/`테크`, `<형식>`은 `객관식`/`주관식`, `<n>`은 1부터 연속 증가. 이 형식과 맞지 않는 헤더는 등장 순번으로 식별해 위반으로 보고한다 |
| R2 | 문항 블록 안에 `위치: `<경로>:<시작>(-<끝>)?`` 행이 1개 이상 있고, 헤더 다음 첫 비어 있지 않은 행이 위치 행이다. 뒤에 ` (base)` 또는 ` ([permalink](<URL>))` 병기를 허용한다. **댓글 모드(`--comment`)에서는 모든 위치 행에 규격에 맞는 permalink가 정확히 1개 필요하며, `--head`/`--base`를 받으면 커밋 SHA를 기준값과 대조한다** |
| R3 | `<details>`~`</details>` 안에 `<summary>힌트</summary>`를 둔 힌트 블록이 정확히 1개이고, 문항 종료 시 접기 깊이가 0이다 (짝이 없는 `</details>`도 위반) |
| R4 | `<summary>정답·해설</summary>`을 둔 정답 블록이 정확히 1개이고 힌트 블록 뒤에 오며, 힌트 블록과 **다른** 접기 블록이다. 블록 안 첫 내용 행이 객관식은 `(<문자>). <해설>` 형식이고 주관식은 모범 답안이며, `근거: <내용>` 행이 1개 이상 있다. 비즈니스 문항은 `정책 근거: <내용>` 행도 둔다. 내용 행은 빈 행·접기 태그 행·렌더링되지 않는 한 행짜리 HTML 주석을 뺀 행이고, 정답 문자 뒤에 공백만 두면 해설이 없는 것으로 본다 |
| R5 | 접기 밖 선택지 행(`- (a) ` 형식)은 첫 `<details>` 앞에만 둔다. 객관식은 그 선택지가 2개 이상이고 라벨이 `(a)`부터 건너뜀·중복 없이 연속 증가하며, 정답 블록 첫 행의 정답 문자가 그 라벨 중 하나다. 주관식은 위치와 무관하게 0개 |
| R6 | 문항 블록 안 `<details>` 밖 행 중 `정답`으로 시작하는 행이 0건 (정답 비노출의 형식 검사) |
| R7 | 문서 골격: `## 대상`·`## 문항` 헤더 각 1개, 문항 블록은 `## 문항` 아래에만, 문항이 1개 이상, `## 응답 기록`(선택)은 마지막 문항 블록 뒤에만 온다. 댓글 모드에서는 `## 응답 기록`을 두지 않는다 |

- 헬퍼 출력 규약: 통과는 무출력·종료 코드 0, 위반은 `위반 R<n> Q<n>: <사유>` 1행씩·종료 코드 1, 사용오류는 종료 코드 2 (`issue-work/scripts/check-clear.sh`와 같은 규약).
- 헬퍼 사용법: `check-quiz.sh <파일>` (대화형 산출), `check-quiz.sh --comment [--head <SHA>] [--base <SHA>] <파일>` (게시 본문). `--head`/`--base`는 `--comment` 없이 쓰면 사용오류다. 일반 모드는 permalink 병기를 선택으로 보므로 게시 본문을 일반 모드로 검사하면 permalink 계약이 검증되지 않는다.
...
- [ ] [D] 시험 실행 산출 파일이 형식 검사를 통과하고 문항 3개 이상·응답 기록 3행 이상이며, `--comment` 본문 파일의 **모든** 위치 행에 기준 커밋 SHA permalink가 있다
  <details>
  <summary>검증 명령 — repo 루트에서 실행, 출력 0건이면 통과</summary>
  ...
  chk=git-review-quiz/scripts/check-quiz.sh
  head=e1543edc177a406c69c92fbf377d2e207a8bec31   # 시험 대상 PR #91의 headRefOid
  "$chk" "$q" || echo '위반: 산출 파일 형식'
  [ "$(grep -cE '^### Q[0-9]+ \[' "$q")" -ge 3 ] || echo '위반: 문항 3개 미만'
  [ "$(awk '/^## 응답 기록/{f=1; next} f && /^\| Q[0-9]+ \|/' "$q" | wc -l)" -ge 3 ] || echo '위반: 응답 기록 3행 미만'
  "$chk" --comment --head "$head" "$c" || echo '위반: 댓글 본문 형식 또는 permalink 계약'
  [ "$(grep -cE '^위치: ' "$c")" \
    -eq "$(grep -cE '^위치: .*\(\[permalink\]\(https://[^)]+/blob/[0-9a-f]{40}/[^)]+#L[0-9]+(-L[0-9]+)?\)\)$' "$c")" ] \
    || echo '위반: permalink 없는 위치 행 존재'
  ```

  - 설계 주의: 두 파일은 `--clear` 5단계에서 정리되므로 Task N 검증은 `--clear` 전에 수행한다.
  - 설계 주의: permalink는 `grep -q`로 1건만 찾지 않고 **위치 행 총수와 permalink 행 총수를 대조**한다. 1건 탐침은 나머지 문항의 permalink가 빠져도 통과하므로, 헬퍼의 `--comment` 검사가 무력화됐을 때 DoD가 독립적으로 잡지 못한다.
  </details>
...
- [ ] [QD] 시험 실행 산출물의 모든 문항이 변경 자체를 묻고(일반 지식 문항 0건), 힌트가 정답을 직접 말하지 않으며, 정답 블록의 근거 위치가 실제 diff와 일치하고 정답·해설의 서술이 소스의 실제 동작과 일치한다  (검증: 교차모델 audit이 산출물을 diff·실제 동작과 대조 채점)  ← 강등 사유: 문항·힌트 내용의 의미 판단이라 명령으로 환원 불가
...
- Task 5의 대화형 시험은 **사용자가 직접 응답**한다. Task 4까지 마친 뒤 대화형 세션을 열고, 사용자가 문항 3개 이상에 답하며 힌트 요청 1회와 의도적 오답 1문항을 수행한다. DoD의 `[ND]` 항목이 사용자 체감 판정이라 AI 자문자답으로는 검증이 성립하지 않는다 (2026-08-29 Task 0 질의 확정).
- Task 1~5는 **Task별로 커밋**한다. 형식은 `git-commit` 규칙(Conventional Commits + 말미 `(#92)`)을 따른다 (2026-08-29 Task 0 질의 확정).
- **근거 문서 "없음" 판정 기준**: `.ai/30_contract/`·`.ai/40_domain/`에 index와 템플릿 자리표시자만 있고 실질 문서가 없으면 "없음"으로 본다. 이 repo는 `40_domain/glossary.md`가 빈 표 템플릿이고 `specs/`·`policies/`가 `.gitkeep`뿐이라 이 판정이 적용되며, Task 5의 "문서 없음 → 테크 문항만" 경로가 재현된다. 파일 존재만으로 근거가 있다고 보면 이 경로가 검증되지 않으므로 SKILL.md 3단계에 이 기준을 명시한다.
- Task 5 시험 실행은 repo의 `git-review-quiz/SKILL.md`를 직접 읽어 그 절차를 수행한다. `install-skills`로 홈 경로에 설치하는 일은 이 이슈 범위 밖이라 슬래시 명령으로 호출하지 않는다.
- R5의 선택지 정규식은 `^- \([a-z]\) `로 일반화한다. 규칙 표의 `- (a) `는 첫 선택지 예시이며, 실제 선택지는 `(a)`·`(b)`·`(c)` 순으로 증가한다. 헬퍼는 개수뿐 아니라 **라벨이 `(a)`부터 건너뜀·중복 없이 연속하는지**도 본다 (2026-08-30 Task N audit F-2 보정).
- 형식 검사 헬퍼는 게시 본문을 별도 모드로 판정한다. `--comment`가 permalink 전수 검사와 `## 응답 기록` 부재를, `--head`/`--base`가 커밋 SHA 대조를 켠다. 대화형 산출에는 permalink가 없으므로 두 모드를 나누지 않으면 한쪽이 항상 실패한다 (2026-08-30 Task N audit F-4 보정).
- 형식 검사 헬퍼는 `## 문항` 아래의 모든 `### ` 헤더를 문항 후보로 연다. 인식 범위 밖 헤더를 조용히 버리면 그 블록의 R2~R6 검사가 통째로 빠지고, 마지막 문항이 깨진 경우 뒤따르는 번호가 없어 연속성 판정으로도 잡히지 않는다. 따라서 `## 문항` 아래에는 문항 외 소절 헤더를 두지 않는다 (2026-08-30 Task N audit F-3 보정).
- 접기 밖 선택지 행은 첫 접기 앞에만 유효하다. 첫 접기 뒤의 선택지는 객관식이면 위치 위반, 주관식이면 개수 위반으로 R5에서 걸린다. 주관식 판정을 첫 접기 앞으로 한정하면 힌트 뒤에 선택지를 둔 문항이 0개 규칙을 우회한다 (2026-08-30 Task N audit 2차 F-6 보정).
- 문항의 정답·해설은 소스 주석의 서술이 아니라 실제 동작을 기준으로 판정한다. 주석이 일반적인 성질을 설명하는데 코드는 그 성질을 피해 가는 인자를 쓰는 경우가 있어, 주석만 근거로 삼으면 정답이 성립하지 않는 문항이 나온다 (2026-08-30 Task N audit 3차 F-7 보정).
- `--comment` 모드의 permalink 검사는 URL 형태(`https://`·`blob/<40자 SHA>`·경로 존재·줄 앵커)와 `--head`/`--base` 기준 SHA만 대조한다. 확정 저장소와 위치 행의 경로·줄 범위 일치는 검사하지 않으며 원장 K-0007로 수용했다 (2026-08-30 Task N audit 2차 F-4, `http` 허용만 보정).
