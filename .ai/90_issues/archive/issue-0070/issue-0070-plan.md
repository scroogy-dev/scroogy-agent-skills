# Issue #70 실행계획 AI-CONTEXT.md·ai-workspace 템플릿 Git 정책 표에 git-pr-feedback 행 추가

> 스펙: [issue-0070-spec.md](./issue-0070-spec.md)
...
- **점검 결과**: 발견 4건을 spec `## 전제 (Assumptions)`에 기록 — 행 삽입 위치(버린 대안 포함), 행 문구 출처(이슈 #70 본문), 템플릿 등재 근거, ai-workspace update 비전파.
...
- [x] 완료
- **목표**: `.ai/AI-CONTEXT.md`의 `## Git 정책` 표 누락을 해소한다.
- **작업 내용**:
  1. `.ai/AI-CONTEXT.md`의 `## Git 정책` 표에서 `/git-pr` 행 바로 아래에 `/git-pr-feedback` 행을 삽입한다 (문구는 spec 전제 참조).
- **완료 기준**:
  - [D] `.ai/AI-CONTEXT.md`의 `## Git 정책` 표 skill 집합·순서가 기대 5종과 일치한다
    <details>
    <summary>검증 명령 — repo 루트에서 실행, 출력 0건이면 통과</summary>

    ```bash
    diff <(awk '/^## Git 정책/{s=1;next} /^## /{s=0} s && /^\| `/{print $2}' .ai/AI-CONTEXT.md) \
         <(printf '%s\n' '`/git-commit`' '`/git-pr`' '`/git-pr-feedback`' '`/git-review-context`' '`/git-review`') \
      >/dev/null || echo '위반: .ai/AI-CONTEXT.md 표 skill 집합·순서 불일치'
    ```
...
- [x] 완료
- **목표**: 신규 설치 repo의 안내도에도 같은 행이 포함되게 한다.
- **작업 내용**:
  1. `ai-workspace/templates/dev/.ai/AI-CONTEXT.md`·`ai-workspace/templates/doc/.ai/AI-CONTEXT.md`의 `## Git 정책` 표에 Task 1과 같은 위치·문구로 행을 삽입한다.
- **완료 기준**:
  - [D] 두 템플릿 각각의 `## Git 정책` 표 skill 집합·순서가 기대 5종과 일치한다
    <details>
    <summary>검증 명령 — repo 루트에서 실행, 출력 0건이면 통과</summary>
  ...
  - [D] 대상 3개 파일의 `## Git 정책` 표 행 본문이 서로 동일하다 — Task 1과 이 Task의 집합·순서 항목이 표 실재를 보장하는 선행 조건이며, 무너지면 빈 추출끼리 비교해 통과처럼 보인다
    <details>
    <summary>검증 명령 — repo 루트에서 실행, 출력 0건이면 통과</summary>
...
- **완료 기준**:
  ...
  - [D] summary의 Task 헤더 집합이 plan과 일치 — 블록 전체 누락·Task N 누락·빈 summary·두 경로 오기를 차단하며, 아래 두 게이트의 선행 조건이라 블록 자체가 없으면 그 두 게이트는 검사 대상이 사라져 그대로 통과한다
    ...
    ```bash
    P=.ai/90_issues/archive/issue-0070/issue-0070-plan.md
    S=.ai/90_issues/archive/issue-0070/issue-0070-summary.md
