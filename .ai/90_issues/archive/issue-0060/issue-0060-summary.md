...
- **특이 사항**: plan의 앵커 유지 검증 임계값 "≥ 8"이 설계 시점 오기로 확인됨(HEAD 실측 7) — issue-work 템플릿 미변경을 git으로 확인한 뒤 plan 기준을 실측값(= 4, = 7)으로 보정 / audit F-1 보정(2026-07-30, 승인 후 반영): 감사 리포트 템플릿의 권장 조치를 접기 밖 본문으로 승격, 접기에는 설명·영향만 유지
...
- **특이 사항**: git-review는 기존에 결과 기록 형식 코드블록이 없어 plan 지시대로 접기 지점을 포함한 형식 섹션을 신설함 / audit F-2 보정(2026-07-30, 승인 후 반영): git-pr PR 메시지 구조·테크 참고사항·작성 예시에 접기 밖 리스크·주의사항 승격 규칙 추가 / audit F-4 보정(2026-07-30, 승인 후 반영): issue-work `--clear` 3단계 이슈 댓글에 결정사항·미해결 리스크·다음 액션을 접기 전 본문 목록으로 승격하는 규칙과 형식 예시 자리 추가
...
- **특이 사항**: 버린 대안(참조 전용 방식)과의 경계를 spec 전제에 명확화 — 내장 기본값 유지로 스킬 독립성은 보존됨 / audit F-3 보정(2026-07-30, 승인 후 반영): spec Out의 분량 예산 문구를 내장 복제 제외·직접 참조 시 적용으로 명확화
