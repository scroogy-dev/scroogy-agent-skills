...
- 분량 예산(요약 200자·항목당 3문장 등)을 내장 폴백 블록에 복제하는 전파 — 단, `writing-principles.md` 파일 직접 참조가 발동하는 repo에서는 분량 예산을 포함한 전체 원칙이 적용된다 (조건부 참조와 내장 전파 구분, audit F-3 보정). 접기 적용 후에도 길이 문제가 남으면 별도 이슈로 다룬다 (2026-07-30 문답에서 확인)
