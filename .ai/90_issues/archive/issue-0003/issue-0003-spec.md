...
- [x] 멀티 repo 워크스페이스에서 임의의 에이전트가 루트 `.ai/AI-CONTEXT.md`만 읽고도 어떤 repo를 봐야 하는지 판단할 수 있는 라우팅 정보가 로비 구조에 명세되어 있다
- [x] 루트 로비 → 개별 repo `.ai/AI-CONTEXT.md` → 코드/문서로 이어지는 탐색 흐름이 두 스킬의 SKILL.md(또는 템플릿)에 명시되어 있다
- [x] `ai-workspace-directory`와 `ai-workspace`의 산출물 항목이 서로 충돌·중복 없이 맞물린다 (양쪽 SKILL.md에서 상호 참조 확인 가능)
- [x] SSOT는 소스코드라는 원칙이 보강 후에도 두 스킬에서 명시적으로 유지된다 (안내판은 라우터일 뿐, 진실의 원천이 아니라는 점 명문화)
- [x] 기존에 생성된 워크스페이스/repo에 대해서도 재실행(재구성/갱신) 시 보강된 구조로 정렬되는 절차가 두 스킬에 반영되어 있다
