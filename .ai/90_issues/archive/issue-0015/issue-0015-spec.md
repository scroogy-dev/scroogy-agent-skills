...
- [x] 정리 옵션의 이름이 결정되어 `issue-work/SKILL.md`에 반영됨
- [x] 옵션 실행 시 이슈 디렉토리가 `active/` → `archive/`로 이동하는 절차가 정의됨
- [x] `.ai/99_workspace/` 정리 절차(정리 대상·보존 대상)가 정의됨
- [x] 이슈 댓글 요약 등록 여부를 사용자에게 질의 후 진행하는 절차가 정의됨
- [x] 기존 `## 이슈 완료 시` 절차와 새 옵션의 관계가 본문에서 모순 없이 설명됨
- [x] 클로드 코워크에서 skill-creator 스킬로 점검 완료 (이슈 참고사항)
