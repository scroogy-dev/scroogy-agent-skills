...
## 열린 결정 (Task 1에서 확정 완료)

- **옵션 이름** → **`--response`로 확정**. 감사 도메인에서 피감사 측 공식 답변을 "(auditee/management) response"라 부르는 정식 용어와 일치하며, `issue-audit`의 자연스러운 짝. 동의·부분동의·반론(변호)·보정 제안을 중립적으로 포괄. (`--feedback`은 대상 모호, `--rebut`은 반박 전용으로 읽혀 탈락)
- **자동탐색 기본 경로** → **인자 없는 플래그로 확정**. 인자 생략 시 `active/` 이슈 번호로 `.ai/99_workspace/issue-<번호>-audit-report.md`를 자동 탐색하고, 리포트가 여러 개면 목록을 제시해 선택받는다. (기존 옵션이 `@<파일>` 인자를 쓰지 않는 패턴과 일치)
- **옵션 + 절차문서 동시 반영 여부** → **둘 다로 확정**. 플래그(호출용) + `issue-workflow.md` 절차(컨텍스트 초기화 후 유지용). (#29 F-1과 같은 이유)
