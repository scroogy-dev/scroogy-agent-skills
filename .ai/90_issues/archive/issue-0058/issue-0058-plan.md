...
- **작업 내용**:
  ...
  5. 포크(fork) 기반 head 지정(`<소유자>:<브랜치>`)·포크 생성·게시 절차는 이 이슈 범위 밖이다 (spec `비포함 (Out)`) — 소스 브랜치는 같은 저장소 기준으로만 서술한다. 다만 포크에서 실행한 경우의 비지원 안내·조기 종료는 서술한다 (3차 audit F-3 반영).
- **완료 기준**:
