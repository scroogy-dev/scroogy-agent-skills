...
- [ ] [D]  기존 active 이슈에 소급 변경이 없다  (검증: 추적 변경 `git diff --name-only main -- .ai/90_issues/active`와 미추적 파일 `git ls-files --others --exclude-standard -- .ai/90_issues/active`의 합산 출력에 `issue-0043/` 외 경로 0건 — `main...` 커밋 비교는 미커밋·미추적 변경을 보지 못해 작업 트리 비교로 교체)
