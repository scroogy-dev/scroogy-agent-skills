...
- 원장 항목 템플릿 신설(`ai-workspace/templates/shared/.ai/70_ledger/ledger-entry-template.md`) — 필수 필드 7종
...
- [x] [QD] 원장 참조 규칙이 발견(audit·PR 리뷰) → 등재 → 재검토(재평가·승격·해소)의 순환에서 반복 보고를 실제로 차단하는 정합 구조다  (검증: 교차모델 audit 채점)  ← 강등 사유: 절차 문서 간 의미 정합성은 문자열 대조로 환원 불가
...
- **원장 index 골격은 ai-workspace가 단독 소유한다** (`templates/shared/.ai/70_ledger/index.md`). 이 repo의 기존 관례가 "index.md 골격은 ai-workspace가 배포하고(30_contract·40_domain·50_adr·60_codebase 전부), 개별 항목 문서 템플릿은 생산 스킬이 보유(context-harvest의 `50_adr-template.md`)"이기 때문. 따라서 issue-work에는 index 템플릿을 두지 않는다 — 같은 내용을 두 스킬이 갖는 SSoT 이중화를 피한다.
- ~~원장 항목 템플릿은 `issue-work/templates/`에 둔다 — 등재 절차의 주체가 `--response`이기 때문.~~ **번복(2026-08-03, 이슈 종료 후 사용자 지적)**: 항목 템플릿도 index 골격과 같이 **ai-workspace가 `templates/shared/.ai/70_ledger/`에서 배포**한다. 원 근거와 위 관례("개별 항목 문서 템플릿은 생산 스킬이 보유")는 **생산 스킬이 하나일 때**(context-harvest의 `50_adr-template.md`) 성립하던 것이고, 원장은 생산자가 issue-work·git-pr-feedback 둘이라 전제가 깨진다. issue-work에 두면 (1) git-pr-feedback이 issue-work 설치를 전제하게 되고(`install-skills`는 스킬 단위 선택 설치), (2) ai-workspace가 배포하는 `70_ledger/index.md`가 특정 스킬 내부 경로를 가리킨다 — 원장을 `90_issues/` 밖으로 뺀 이 이슈의 결정과도 어긋난다. 검토 후 버린 대안은 그대로 유효: `.ai/20_templates/` 배치(스킬은 `.ai/`에 의존하지 않고 단독 실행 가능해야 한다는 스킬 독립성 원칙 위반), 신규 스킬 신설(이슈 본문에서 배제 확정), 두 스킬이 각각 사본 보유(형식 이중화).
