...
- **작업 내용**:
  ...
  4. `### 제약 검증`의 검증 역할에 **역량 하한**(구현에 배정한 모델과 최소 동급 이상, 조건 만족 후보 중 최소)을 추가하고, 만족 불가 시 상위 대체 없이 실패로 안내함을 명시한다. `## 출력`의 제약 미충족 분기에 역량 미달 경로를 포함시킨다. (3차 audit F-1 보정으로 확장된 작업 — 근거는 spec `## 전제 (Assumptions)`)
- **완료 기준**:
