...
- [x] [D] spec 템플릿 `완료의 정의` 예시에 접기 구조가 실재  (검증: repo 루트에서 `grep -c '<details>' issue-work/templates/issue-spec-template.md` ≥ 1 그리고 `grep -c '</details>' issue-work/templates/issue-spec-template.md`가 같은 값)
- [x] [D] plan 템플릿의 일반 Task 예시와 Task N 고정 블록 양쪽에 접기 구조가 실재  (검증: repo 루트에서 `awk '/^### Task N/{n=1} /<details>/{if(n)tn++; else t++} END{print (t>=1 && tn>=1) ? 0 : 1}' issue-work/templates/issue-plan-template.md` 출력 0)
- [x] [D] SKILL.md에 완료 기준 형식 규칙이 기재  (검증: repo 루트에서 `grep -c '완료 기준 형식' issue-work/SKILL.md` ≥ 1 — 구현 시 이 문구를 규칙 앵커로 사용)
- [x] [D] 기존 `[D]` 앵커 기반 검증 명령이 새 형식에서도 통과  (검증: `bash issue-work/tests/run-tests.sh` 종료 코드 0, 출력 마지막 행 `failed: 0`)
- [x] [QD] 본문·접기 배치가 제안 형식과 일치하고 접기 금지 기준(writing-principles: 결정사항·리스크·액션 아이템)과 불일치 없음  (검증: 교차모델 audit이 채점)  ← 강등 사유: 형식 일치·규칙 간 정합은 의미 대조라 명령으로 환원 불가
- [x] [ND] 리스트 항목 하위에 들여쓴 `<details>`가 GitHub에서 접힘/펼침으로 정상 렌더링  (검증: 사람이 GitHub 파일 뷰에서 확인)  ← 강등 사유: 렌더링 결과는 화면 판정이라 명령으로 환원 불가
...
- `run-tests.sh`의 게이트 추출은 접기 안 bash 코드 블록에서 `P=`/`S=` 경로 할당 행을 **제외하고** 뽑는다 — fixture 경로 주입은 현행 계약(`run_gate`·`assert_pair_gate`가 로컬 변수로 세팅)을 그대로 유지한다. 코드 블록을 통째로 뽑아 경로를 `sed`로 치환하는 대안은 경로 주입 경로가 이중화되어 기각했다 (사용자 합의).
- 템플릿 예시(spec·plan의 예시 Task)의 접기 안에는 **동작하는 짧은 예시 명령**을 넣고 본문 문장은 placeholder를 유지한다 — 한 줄 placeholder를 접으면 "긴 명령을 접는다"는 형식의 의도가 예시로 드러나지 않기 때문이다 (사용자 합의). Task N 고정 블록은 원래 실제 명령이라 해당 없다.
- plan 템플릿 `## Tasks` 안내 blockquote도 새 형식 규칙을 반영한다 — 안내문이 완료 기준 형식을 서술하므로, 갱신하지 않으면 안내와 예시가 어긋난다.
