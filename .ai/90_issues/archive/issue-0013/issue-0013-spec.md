...
- [x] 권고 1~7이 해당 스킬의 SKILL.md(및 분리 시 `references/`)에 반영됨
- [x] 권고 8(교차 중복 단일 출처화)에 대한 설계 판단이 기록됨 (반영 또는 보류 사유)
- [x] 분리·신설된 파일이 각 스킬 본문에서 포인터로 연결되어 단독 실행 가능성이 유지됨
- [x] 사용자가 클로드 코워크에서 재점검하여 추가 개선 사항이 없음을 확인함
