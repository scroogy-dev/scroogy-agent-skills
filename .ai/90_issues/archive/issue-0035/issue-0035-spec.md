...
- 스킬 repo 판별 가드 — 임의의 cwd에서 실수로 실행해 엉뚱한 디렉토리를 소스로 삼는 오설치를 막는 가벼운 가드를 둔다. 기존 "스캔 목록 제시 → 사용자 선택" 절차는 유지한다. 단일 스킬 repo도 지원 대상이므로 가드는 `*/SKILL.md` 0건 차단(1건 이상 통과)으로 설계하고, 1건만 매칭되면 사용자 확인을 거친다. (audit F-1·F-3 승인 보정으로 명확화)
...
- [ ] [D]  스킬 repo 판별 가드가 SKILL.md에 추가됨  (검증: 가드 로직 문구가 `grep`으로 확인됨 — 예: `*/SKILL.md` 1건 이상 존재 확인, 0건이면 중단. 원문 "복수 존재"는 audit F-1·F-3 승인 보정으로 정합화)
...
- [ ] [D]  가드·자기제외 스캔·헬퍼 탐색 스니펫이 SKILL.md 본문에서 추출되어 픽스처 스모크 테스트됨 — 문서-구현 드리프트 감지  (검증: `bash install-skills/tests/run-tests.sh` 0 실패, 스니펫 테스트 포함) (audit F-2 승인 보정)
