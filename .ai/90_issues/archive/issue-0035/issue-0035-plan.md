...
- **작업 내용**:
  1. 스캔 절차(58줄) 앞단에, 루트에 `*/SKILL.md`가 1건 이상 존재하는지(0건이면 중단) 확인하는 가벼운 가드를 SKILL.md에 추가한다. (원문 "복수 존재"는 audit F-1·F-3 승인 보정으로 정합화 — 단일 스킬 repo도 지원 대상)
...
### Task 8: 교차모델 audit 발견사항 보정 (F-1·F-2·F-3 — 사용자 승인분)

- [x] 완료
- **목표**: 교차모델 audit 리포트(OpenAI, GPT-5.5)의 발견사항을 `--response` 게이트에서 사용자가 승인한 방향으로 보정한다.
- **작업 내용**:
  1. F-1·F-3 (승인: 수정안 A): 가드의 0건 차단(1건 이상 통과)을 설계 의도로 확정 — spec·plan의 "복수 존재 확인" 문구를 "1건 이상 존재 확인"으로 정합화하고, spec에 단일 스킬 repo 지원 의도를 명시하며, SKILL.md 가드에 "1건만 매칭되면 사용자 확인 후 진행" 문구를 보강한다. (≥2 강화는 반려 — 후속 이슈로 복제본이 제거되면 `scroogy-content-skills`가 단일 스킬 repo가 되어 차단되는 부작용)
  2. F-2 (승인: 이번 이슈에서 반영): SKILL.md의 가드·자기제외 스캔·헬퍼 탐색 스니펫을 본문에서 추출해 픽스처에서 실행하는 스모크 테스트를 `tests/run-tests.sh`에 추가한다 (문서-구현 드리프트 감지).
- **완료 기준**: `grep -q '1건 이상' install-skills/SKILL.md` + `grep -q '1건만 매칭되면' install-skills/SKILL.md` + `bash install-skills/tests/run-tests.sh` 0 실패(스니펫 스모크 테스트 포함).
- **근거**: `.ai/99_workspace/issue-0035-audit-report.md` F-1(MEDIUM)·F-2(LOW)·F-3(INFO), 사용자 항목별 승인 (2026-07-02).
