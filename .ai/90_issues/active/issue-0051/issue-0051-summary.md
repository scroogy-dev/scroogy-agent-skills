...
- **보정 반영**: 0건
- **재시도**: 0회
- **수행 내용 요약**: `issue-model-triage/SKILL.md`를 spec Task 2 구조(개요·관련 skill·참조 문서·입력·판단 로직·출력·예시)로 작성. 헤더 명칭은 spec D2~D9 검증 명령과 한 몸이라 그대로 사용. 입력 3경로(인자·프롬프트·미입력 시 질의), 판단 로직(분류 기준 4신호 표 + 제약 검증 + 만족 불가 시 안내), 출력 3요소(추천 표·근거 요약·자동 실행 없음 경계), 예시 2종(후보 3개 추천 / 타벤더 부재 실패)을 담음. 결정적 검증 D2·D3·D4·D5·D7·D9 전부 통과.
- **특이 사항**: 모델명 하드코딩 금지(QD) 대응 — 판단 로직·출력 절에는 모델명을 두지 않고, 벤더 추정 예시는 입력 절로, 구체 모델명은 예시 절로만 한정. D6·D8·D10은 교차모델 audit 채점 대상.
