...
- **작업 내용**:
  ...
  3. spec·plan 본문의 `<스킬명>` placeholder를 확정 스킬명으로 전부 치환한다.
- **완료 기준**:
...
- **작업 내용**:
  1. `<스킬명>/SKILL.md`를 아래 구조로 작성한다 (헤더 명칭은 spec D2~D9 검증 명령과 한 몸이므로 그대로 쓴다):
     - 프론트매터: `name: <스킬명>`(디렉토리명과 일치), `description:` 한 줄(트리거 키워드 포함, 한국어)
     - `## 개요` — 목적(이슈 시작 전 모델 조합 트리아지)과 경량성
     - `## 관련 skill` — issue-work("새 이슈 시작 시" 앞단에서 활용 가능, 절차 결합 아님)·issue-audit(검증 타벤더 제약 근거)
     - `## 입력` — 하위 절 `### 인자 입력`(예: `/<스킬명> fable5, opus4.8, gpt-5.6sol`) / `### 프롬프트 입력` / `### 미입력 시 질의`. 모델 목록 하드코딩 금지, 모델명에서 벤더 추정하되 불확실하면 사용자 확인
