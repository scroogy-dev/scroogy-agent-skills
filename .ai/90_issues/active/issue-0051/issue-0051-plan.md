...
- **완료 기준**:
  ...
  - [D] spec·plan에 placeholder 잔존 0건  (검증: repo 루트에서 `grep -rn '<스킬''명>' .ai/90_issues/active/issue-0051/issue-0051-spec.md .ai/90_issues/active/issue-0051/issue-0051-plan.md` 출력 0건. 검사 대상을 치환 대상인 spec·plan 두 파일로 한정한다 — summary는 치환 이력을 서술하는 문서라 placeholder 토큰을 정당하게 언급하므로 검사 범위에서 제외한다. 패턴을 따옴표로 쪼갠 것은 이 완료 기준 행 자신이 검색에 걸리는 자기 참조를 막기 위함이다)
