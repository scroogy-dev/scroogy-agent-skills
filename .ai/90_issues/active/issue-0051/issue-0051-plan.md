...
- **완료 기준**:
  ...
  - [D] 이슈 문서에 placeholder 잔존 0건  (검증: repo 루트에서 `grep -rn '<스킬''명>' .ai/90_issues/active/issue-0051/` 출력 0건. 패턴을 따옴표로 쪼갠 것은 이 완료 기준 행 자신이 검색에 걸리는 자기 참조를 막기 위함이다)
...
- **작업 내용**:
  1. `issue-model-triage/SKILL.md`를 아래 구조로 작성한다 (헤더 명칭은 spec D2~D9 검증 명령과 한 몸이므로 그대로 쓴다):
     ...
     - `## 예시` — `### 예시 1:` 후보 3개(fable5, opus4.8, gpt-5.6sol) 입력 시 추천 표 / `### 예시 2:` 타벤더 검증 모델 부재 시 실패 안내
