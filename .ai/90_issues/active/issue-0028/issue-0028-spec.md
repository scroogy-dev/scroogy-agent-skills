...
- 구 경로(`~/.gemini/antigravity/skills/`) 레거시 마이그레이션: **심링크는 보존**(신 경로와 동일), **실제 디렉토리로 잔존하면 경고·정리 제안**. 경로 교체만으로는 `--clear`가 신(대상) 경로만 비워 구 경로 잔존물이 남고 중복 인식될 수 있으므로 install-skills가 명시적으로 처리한다.
...
- [ ] [D]  레거시 구 경로가 **실제 디렉토리로 잔존**하면 FAIL, **심링크/부재**는 PASS로 판정  (검증: `install-skills/tests/` 러너의 레거시 케이스, 0 실패)
