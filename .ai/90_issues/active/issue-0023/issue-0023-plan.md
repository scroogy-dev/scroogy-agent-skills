# Issue #23 실행계획 스킬 결정적 헬퍼 테스트 규칙 정립 및 install-skills 배포 제외
...
- **작업 내용**:
  ...
  3. 개요/참고에 "배포 시 `tests/` 등 개발 전용 경로 제외" **자기 동작**을 한 줄 명시(ADR 링크). 제외 패턴 목록은 이 파일에만 둔다.
...
- **작업 내용**:
  ...
  2. **결정**: 헬퍼는 `scripts/`, 테스트는 `<skill>/tests/`에 함께 둠(테스트 데이터 `tests/` 안에 중첩) · 스크립트 언어에 맞춘 경량 러너(외부 의존성 없이) · 기대 출력·고정 입력 데이터 비교 방식 · 배포 시 `tests/` 제외.
  3. **근거**: 스킬 독립성 + 점진적 공개(번들 리소스는 읽기 전까지 컨텍스트 비용 0) + 배포 용량(install-skills `cp -r`는 통째 복사) 의 세 축 분리.
  4. **기각안**: 루트 중앙집중 `tests/`(독립성 깨짐) · 테스트 미배포(제거 안 함, 용량 부담) · JUnit/maven 도입(헬퍼 규모 대비 과함).
  5. "무엇을 어떻게 제외하는지"는 **`install-skills/SKILL.md`를 참조**로 가리킨다 — 제외 패턴 목록을 ADR에 복제하지 않는다(불일치 방지).
...
- **작업 내용**:
  1. `.ai/AI-CONTEXT.md` "스킬 작성 규칙"에 포인터 한 줄 추가: "테스트는 `<skill>/tests/`에 두며(테스트 데이터 중첩), `install-skills` 배포 시 제외됨 → `install-skills/SKILL.md`, ADR".
