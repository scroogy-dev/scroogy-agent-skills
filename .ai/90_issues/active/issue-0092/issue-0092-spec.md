...
| R4 | `<summary>정답·해설</summary>`을 둔 정답 블록이 정확히 1개이고 힌트 블록 뒤에 오며, 힌트 블록과 **다른** 접기 블록이다 |
| R5 | 접기 밖 선택지 행(`- (a) ` 형식)은 첫 `<details>` 앞에만 둔다. 객관식은 그 선택지가 2개 이상이고 라벨이 `(a)`부터 건너뜀·중복 없이 연속 증가한다. 주관식은 위치와 무관하게 0개 |
