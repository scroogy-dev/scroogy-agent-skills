...
- **작업 내용**:
  ...
  3. `### 제약 검증`에서 "플랜·구현 같은 모델 기본" 제약과 캐시 근거를 삭제하고, Task 0에서 확정된 벤더 제약 처리 방향을 반영한다. 검증 역할 제약(구현과 다른 모델 + 다른 벤더)은 그대로 둔다.
- **완료 기준**:
  - [D] `### 구현 난이도 평가` 헤더 행 정확히 1개  (검증: repo 루트에서 `grep -cE '^### 구현 난이도 평가$' issue-model-triage/SKILL.md` 출력 1)
  - [D] `## 판단 로직`부터 `## 출력` 헤더까지의 구간에 캐시 우선 규칙 문구 잔존 0건  (검증: repo 루트에서 `sed -n '/^## 판단 로직/,/^## 출력/p' issue-model-triage/SKILL.md | grep -cE '캐시|같은 모델을 기본|플랜·구현 같은 모델'` 출력 0. 예시 절은 Task 2에서 정리하므로 이 단계는 구간을 한정한다)
...
- **완료 기준**:
  - [D] SKILL.md 전체에서 캐시 우선 규칙 문구 잔존 0건  (검증: repo 루트에서 `grep -cE '캐시|같은 모델을 기본|플랜·구현 같은 모델' issue-model-triage/SKILL.md` 출력 0)
