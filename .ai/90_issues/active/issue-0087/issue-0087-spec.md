...
- `.ai/90_issues/` 하위 (이슈 수명주기 산출물이라 일괄 정비 대상이 아님. `issue-workflow.md` 사본은 템플릿 동기화로만 갱신)
...
- [ ] [D] 스캔 스코프 전체에서 grep 가능한 금지 패턴(em dash `—`, 부정 대조 "가 아니라") 잔존이 예외 목록 등재 건수와 정확히 일치한다 (미등재 잔존 0건). 잔존 건수는 패턴 포함 행 수(`grep -c`) 기준이다
  <details>
...
- [ ] [D] 문체 수정한 파일의 문서 구조(헤더·체크박스·코드 펜스·표 행 수)가 main 대비 불변이다. `70_ledger/`는 수용 등재로 표 행·항목 파일이 정당하게 늘어나므로 제외한다
  <details>
  <summary>검증 명령 — repo 루트에서 실행, 출력 0건이면 통과</summary>
...
- [ ] [D] 동기화 동일 쌍 10종이 정합하다 (템플릿이 SSoT, 사본과 diff 일치). 상이 쌍 3종(`40_domain/index.md`, `50_adr/index.md`, `70_ledger/index.md`)은 repo 고유 행이 있어 이 검증에서 제외하고 아래 [QD]로 넘긴다
  <details>
  <summary>검증 명령 — repo 루트에서 실행, 출력 0건이면 통과</summary>
...
- [ ] [D] 예외 목록에서 분류가 `수용`인 행은 전부 원장 항목(`K-<번호>`)을 참조한다
  <details>
  <summary>검증 명령 — repo 루트에서 실행, 출력 0건이면 통과</summary>
...
- em dash 심사 기준. 규칙이 금지하는 용법은 삽입구·절 연결이다. 리스트 항목의 "라벨 — 부연" 구분자, 접기 제목 기본형("검증 명령 — 출력 0건이면 통과") 같은 형식 고정 문구는 위반이 아니며 예외 목록에 기록만 한다. 기준 시점 잔존 553행(61개 파일) 중 상당수가 이 유형이라 실제 수정량은 전수보다 작다.
...
- 패턴 잔존 건수는 패턴 포함 행 수(`grep -c`) 기준으로 세며, 예외 목록의 허용 건수도 같은 기준이다.
