...
- summary 구성: `모델 기록` 표(계획·계획 audit·구현·최종 audit 4행, 모델·effort 열), `계획 감사` 줄, Task별 `결과`·`수행 모델`·`수행 effort`·`audit 발견`·`보정 반영`·`재시도`. Task 0·일반 Task 블록의 지표 5종은 값이 없어도 필드를 남긴다. effort는 도구가 기록한 표기 그대로 적고(확인 불가면 `-`) 벤더끼리 비교하지 않으며 게이트 조건에 쓰지 않는다. 수치 지표는 이슈 전체 누적이며 한 발견은 주 Task 하나에만 귀속한다.
