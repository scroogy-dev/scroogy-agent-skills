...
- 리포트 상단 메타에 `> 감사 모델: <벤더, 모델명>`, `> 감사 effort: <도구가 기록한 표기, 확인 불가면 ->`와 감사 회차를 적는다. `감사 effort` 줄 값은 summary `모델 기록` 표의 해당 audit 행 `effort` 열로 옮겨 적으며 벤더끼리 비교하지 않는다. 이전 회차 리포트는 `issue-<번호>-audit-report-<회차>.md`로 보존하고, archive 재감사 시 archive의 최신 리포트도 같은 규칙으로 회전한다.
