...
1. **링크 무결성 점검**: Q1=(a)를 선택했는데 `LICENSE` 파일이 디렉토리에 없으면, `(./LICENSE)` 링크가 깨진 상태가 됩니다. 이 경우 사용자에게 **LICENSE 파일을 별도로 생성하도록 권유**합니다 (스킬은 LICENSE 파일을 생성하지 않음).
2. `README.md`로 저장합니다. 기존 파일이 있으면 덮어쓸지 1회 확인합니다.
