...
1. **LICENSE/NOTICE 파일 생성**: Q1-2에서 "생성"을 골랐다면 [LICENSE 파일 생성 규칙](#license-파일-생성-규칙)에 따라 LICENSE를 만듭니다. Apache 2.0이면 `NOTICE`도 함께 처리합니다. `--force-license`가 명시되어 있으면 [`--force-license` 플래그](#--force-license-플래그) 가드를 통과한 경우에만 기존 파일을 덮어씁니다.
2. **링크 무결성 점검**: Q1=(a)인데도 `LICENSE` 파일이 디렉토리에 없으면 (Q1-2에서 "생성하지 않음"을 골랐거나 가드로 스킵된 경우), `(./LICENSE)` 링크가 깨진 상태가 됩니다. 이 경우 사용자에게 LICENSE 파일을 별도로 생성하도록 권유합니다.
