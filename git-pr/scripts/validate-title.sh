...
#
# 판정 항목:
#   - 제목이 한 줄인가 (말미 개행 1개만 허용, 제목 안의 LF·CR 은 거부)
...
if [ -n "$file" ]; then
  ...
  # 제출은 `--title "$(cat <제목 파일>)"` 로 파일 전체를 넘긴다. 첫 줄만 읽으면 검증 대상과
  # 제출 대상의 범위가 달라져, 둘째 줄부터의 내용이 검증 없이 그대로 게시된다.
  # 명령 치환은 말미 개행을 지우므로 `printf x` 로 원문 끝을 보존해 읽는다.
  raw="$(cat "$file"; printf 'x')"
  raw="${raw%x}"
else
  raw="$title_arg"
fi
...
# 말미 개행 1개는 텍스트 파일 관례라 허용하고, 그 밖의 줄바꿈은 거부한다.
# 이후 형식 검사는 첫 줄로 이어 가며 사유를 한 번에 보고한다.
title="${raw%$'\n'}"
if [ "$title" != "${title%%$'\n'*}" ]; then
  ng "제목이 여러 줄입니다 — PR 제목은 한 줄이어야 합니다: ${title%%$'\n'*} …"
  title="${title%%$'\n'*}"
fi
case "$title" in
  *$'\r'*)
    ng "제목에 CR 문자가 있습니다 — 줄바꿈 없는 한 줄로 적습니다: $title"
    title="${title//$'\r'/}" ;;
esac
