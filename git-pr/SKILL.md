...
* **호출 흐름:** `.ai/60_codebase/index.md`에 내용이 있으면 참고하되, SSoT는 소스코드이므로 반드시 실제 코드를 확인하여 작성합니다. 색인과 실제 코드가 다른 부분을 발견하면 참고사항에 "코드베이스 색인 갱신이 필요해 보입니다 (`/code-map`)" 의견을 남깁니다. 기능(엔트리 포인트)별로 나누어, 주요 컴포넌트 간의 실행 흐름을 디렉토리 트리 형태의 ASCII 다이어그램으로 도식화합니다. 각 노드에 클래스명과 메서드명을 명시하고(`ClassName#methodName`), 들여쓰기와 트리 기호(`├──`, `└──`)로 호출 계층을 표현합니다. 핵심 로직은 노드 옆에 `# 설명` 주석 형태로 간략히 기재합니다. 하나의 이슈에 여러 기능이 있는 경우 기능별로 각각 작성합니다.
