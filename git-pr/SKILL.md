...
- 소스 브랜치가 원격에 게시되지 않았으면 push 여부도 승인 대상에 포함해 이 단계에서 게시 상태를 확정합니다. `gh`가 push 위치·fork 생성을 대화형으로 되물으면 비대화형 실행이 멈춥니다.
...
- 생성 수단은 `gh` CLI를 기본으로 하고, `gh`가 없거나 인증되지 않았으면 GitHub MCP `create_pull_request`로 폴백합니다. 둘 다 쓸 수 없으면 그 사실을 알리고 최종 제목·본문을 남긴 채 종료합니다.
  ...
  ```bash
  # 정식 PR
  gh pr create --base <베이스 브랜치> --head <승인한 소스 브랜치> --title "<제목>" --body-file <본문 파일>

  # 드래프트 PR
  gh pr create --draft --base <베이스 브랜치> --head <승인한 소스 브랜치> --title "<제목>" --body-file <본문 파일>
  ```

- `--head`는 생략하지 않습니다 — 생략하면 실행 시점의 현재 브랜치가 쓰여 승인받은 대상과 어긋날 수 있습니다.
- GitHub MCP로 폴백할 때는 `create_pull_request`의 `draft` 인자를 승인받은 유형에 맞추고, `head`도 같은 소스 브랜치로 전달합니다.
